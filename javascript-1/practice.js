...
colorCopyAdd = colorCopy.push("blue")
...
const total = myNumbers.reduce(function(acc, num){
	return acc + num
})
...
myNumbers.forEach(function(num){
  let newArray = []
  newArray = myNumbers.indexOf(num)
  myNumbersIndex.push(newArray)
})
...
notGeorge.map(function(num){
	num = "George"
	forTheLoveOfGeorge.push(num)
})
...
let totallyAwesome = people.reduce(function(acc, num){
	return acc + num.awesomeLevel
}, 0)
